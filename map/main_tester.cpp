...
void testOperations()
{
              ...
              << "] --------------------]\t\t\033[0m";
    {
        ...
        /*---------------------------------- time limit test --------------------------------------------*/
        {
            ...
            start = get_time();
            res = m.lower_bound(50)->first;
            end = get_time();
            diff = end - start;
            diff = (diff) ? (diff * TIME_FAC) : TIME_FAC;

            ualarm(diff * 1e3, 0);
            ft_res = ft_m.lower_bound(50)->first;
            ualarm(0, 0);
            cond = ft_res == res;
        }
        std::map<int, std::string> m;
              ...
              << "] --------------------]\t\t\033[0m";
    {
        ...
        /*---------------------------------- time limit test --------------------------------------------*/
        {
            ...
            start = get_time();
            res = m.upper_bound(50)->first;
            end = get_time();
            diff = end - start;
            diff = (diff) ? (diff * TIME_FAC) : TIME_FAC;

            ualarm(diff * 1e3, 0);
            ft_res = ft_m.upper_bound(50)->first;
            ualarm(0, 0);
            cond = ft_res == res;
        }
        std::map<int, std::string> m;
              ...
              << "] --------------------]\t\t\033[0m";
    {
        ...
        /*---------------------------------- time limit test --------------------------------------------*/
        {
            ...
            start = get_time();
            res = m.equal_range(50);
            end = get_time();
            diff = end - start;
            diff = (diff) ? (diff * TIME_FAC) : TIME_FAC;

            ualarm(diff * 1e3, 0);
            ft_res = ft_m.equal_range(50);
            ualarm(0, 0);
